import{T as M,o as U,L as x}from"./assets/main-CY_fChHm.js";import{i as m}from"./assets/vendor-CTT6gwRZ.js";document.addEventListener("DOMContentLoaded",async function(){const e=document.getElementById("weekly-cards"),t=new M,a=document.getElementById("loader-weekly"),s={"Science Fiction":"Sci-Fi"},o=document.querySelector(".backdrop");function c(){o.classList.remove("is-visible"),o.classList.add("is-closed")}o.addEventListener("click",r=>{r.target===o&&c()});try{const r=await t.getTrendingMovies("week");async function d(){const g=window.innerWidth;let p=2;g<=600&&(p=1),e.innerHTML="";for(const n of r.slice(0,3)){const i=document.createElement("div");i.classList.add("card");const E=n.id,$=n.poster_path?`https://image.tmdb.org/t/p/w500${n.poster_path}`:"/img/poster-placeholder.jpg",I=n.title,S=n.release_date?new Date(n.release_date).getFullYear():"Unknown",u=Math.round(n.vote_average*10)/10,_=await t.getMovieGenres(),A=n.genre_ids.slice(0,p).map(T=>{const f=_.find(F=>F.id===T),b=f?f.name:"Unknown";return s[b]||b}).join(", "),B=5,v=Math.floor(u/2),y=u%2>=1?1:0,C=B-v-y,D=[...Array(v).fill('<svg class="star full"><use xlink:href="#icon-star"></use></svg>'),...Array(y).fill('<svg class="star half"><use xlink:href="#icon-star-half"></use></svg>'),...Array(C).fill('<svg class="star empty"><use xlink:href="#icon-star-outline"></use></svg>')].join("");i.style.backgroundImage=`url(${$})`,i.style.backgroundSize="cover",i.style.backgroundPosition="center",i.dataset.id=E,i.innerHTML=`
